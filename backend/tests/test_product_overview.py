...
def test_same_origin_frontends_are_served() -> None:
    ...
    assert "vinyrd-crest-v3.webp" in staff.text
    ...
    assert favicon.headers["location"] == "/staff/assets/vinyrd-crest-v3.webp"

    landing = client.get("/member/")
    assert landing.status_code == 200
    assert "Belong deeper" in landing.text
    assert "vinyrd-crest-v3.webp" in landing.text
...
def test_master_brand_assets_are_served() -> None:
    ...
    for path in (
        ...
        "/staff/assets/vinyrd-crest-v3.webp",
        "/member/assets/vinyrd-full-logo.webp",
        "/member/assets/vinyrd-crest-v3.webp",
    ):
